    ...
    return (
      <div className="space-y-6 pb-8 lg:pb-4">
        ...
        <div className="bento-card flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-white/40 text-lg">Start tracking habits to unlock insights</p>
          <p className="text-white/30 text-sm mt-2">Complete your first habit today to see patterns emerge</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bento-card">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold text-white/80">Completion Rate</h3>
          </div>
          <p className="text-4xl font-bold text-white">{Math.round(analysis.completionRate * 100)}%</p>
          <p className="text-xs text-white/40 mt-1">Across {analysis.totalDays} days</p>
          <div className="mt-4">
            <MiniBarChart data={analysis.weeklyTrend} />
            <div className="flex justify-between mt-1">
              <span className="text-[0.6rem] text-white/20">8 weeks ago</span>
              <span className="text-[0.6rem] text-white/20">This week</span>
            </div>
          </div>
        </div>
        ...
        <div className="bento-card">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold text-white/80">Longest Streak</h3>
          </div>
          <p className="text-4xl font-bold text-amber-400">{analysis.longestStreak.streak}</p>
          <p className="text-xs text-white/40 mt-1">{analysis.longestStreak.habitTitle}</p>
          <div className="mt-4 space-y-3">
            <div className="flex items-center justify-between py-2 border-b border-white/5">
              <span className="text-xs text-white/40">Best Day</span>
              <span className="text-xs text-white/70 font-medium">{analysis.bestDay.day} ({Math.round(analysis.bestDay.rate * 100)}%)</span>
            </div>
            <div className="flex items-center justify-between py-2">
              <span className="text-xs text-white/40">Most Consistent</span>
              <span className="text-xs text-white/70 font-medium">{analysis.mostConsistentHabit.habitTitle}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {analysis.habitPairs.length > 0 && (
        <div className="bento-card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-white/80">Habit Coupling</h3>
          </div>
          <p className="text-xs text-white/40 mb-3">Habits you tend to complete together</p>
            ...
            {analysis.habitPairs.slice(0, 3).map((pair, index) => (
              <div key={index} className="flex items-center gap-3 p-3 rounded-xl bg-white/5">
                ...
                <div className="flex-shrink-0 text-right">
                  ...
                  <p className="text-[0.6rem] text-white/30">{pair.coCompleted}/{pair.totalDays} days</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {analysis.habitsNeedingAttention.length > 0 && (
        <div className="bento-card border-orange-500/20">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-white/80">Needs Attention</h3>
          </div>
          <div className="space-y-3">
            {analysis.habitsNeedingAttention.map((habit, index) => (
              <div key={index} className="flex items-center gap-3 p-3 rounded-xl bg-orange-500/5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white/80 truncate">{habit.habitTitle}</p>
                  <p className="text-xs text-white/40">{Math.round(habit.rate * 100)}% completion rate</p>
                </div>
              </div>
            ))}
          </div>
